<?php
    ...
    $db_passwd = "12341234";
    $db_name = "test";
    $connect = mysqli_connect($db_host,$db_user,$db_passwd,$db_name); //host,username,password == mysql -u[username] -p[password] -h[host]
    $list_result = mysqli_query($connect, 'SELECT * FROM testdb');
     
    if(!empty($_GET['id'])) {
        $topic_result = mysqli_query($connect, 'SELECT * FROM topic WHERE id = '.mysqli_real_escape_string($connect, $_GET['id']));
